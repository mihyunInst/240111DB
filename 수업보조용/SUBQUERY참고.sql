...
-- 메인쿼리(직원의 이름, 직급코드, 급여 조회)
SELECT EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE;

--서브쿼리(직급별 급여 평균)
SELECT AVG(SALARY) FROM EMPLOYEE
WHERE JOB_CODE = 'J1'; -- J1, J2, J3.. 한번씩 쳐보며 평균구해보기

/* 1. 메인쿼리에서 우선 1행을 읽음 (선동일)
 * --> 수행결과 : 선동일은 JOB_CODE가 'J1'이고, SALARY가 8백만임
 * 
 * 2. 서브쿼리에서 선동일의 직급인 'J1'의 평균 급여를 조회함
 * --> 수행결과 : 'J1'의 평균 급여는 8백만임
 * 
 * 3. 메인쿼리가 서브쿼리의 수행결과를 이용하여 
 * 		선동일이 평균 급여보다 초과하여 받는지 판별함
 * 		-->선동일은 J1이지만, 평균급여보다 초과하여 받지 않음(800[급여] = 800[평균])
 * 	
 ...
 * 다음..----------------------------------------------------------
 *
 * 1. 메인쿼리에서 다음 1행을 읽음 (송종기)
 * --> 수행결과 : 송종기는 JOB_CODE가 'J2'이고, SALARY가 6백만임
 * 
 * 2. 서브쿼리에서 송종기의 직급인 'J2'의 평균 급여를 조회함
 * --> 수행결과 : 'J2'의 평균급여는 485만임
 * 
 * 3. 메인쿼리가 서브쿼리의 수행결과를 이용하여 
 * 		송종기가 평균 급여보다 초과하여 받는지 판별함
 * 	--> 송종기는 J2이고, 평균급여보다 초과하여 받음(600[급여] > 485[평균])
 * 
 * 결론 : 송종기는 조건을 만족하므로 메인쿼리가 
 * 				송종기의 EMP_NAME, JOB_CODE, SALARY를 반환
 * 				--> RESULT SET에 포함됨
 * 
 * 다음..----------------------------------------------------------
 * 
 * 1. 메인쿼리에서 다음 1행을 읽음 (노옹철)
 * --> 노옹철 JOB_CODE가 'J2'이고, SALARY가 370만임
 * 
 * 2. 서브쿼리에서 노옹철의 직급인 'J2'의 평균 급여를 조회함
 * --> 'J2'의 평균급여는 485만임
 * 
 * 3. 메인쿼리가 서브쿼리의 수행결과를 이용하여 
 * 		노옹철이 평균 급여보다 초과하여 받는지 판별함
 * 		--> 노옹철은 J2이지만, 평균급여보다 초과하여 받지않음(370[급여] < 485[평균])
 * 	
 * 결론 : 노옹철은 조건을 만족하지 않으므로 메인쿼리가 
 * 				노옹철을 RESULT SET에서 제외함
 * 
 * 다음..----------------------------------------------------------
 * 
 * 1. 메인쿼리에서 다음 1행을 읽음 (송은희)
 * --> 송은희의 JOB_CODE가 'J4'이고, SALARY가 280만임
 * 
 * 2. 서브쿼리에서 송은희의 직급인 'J4'의 평균 급여를 조회함
 * --> 'J4'의 평균급여는 233만임
 * 
 * 3. 메인쿼리가 서브쿼리의 수행결과를 이용하여 
 * 		송은희가 평균 급여보다 초과하여 받는지 판별함
 * 	--> 송은희는 J4이고, 평균급여보다 초과하여 받음(280[급여] > 233[평균])
 * 
 * 결론 : 송은희는 조건을 만족하므로 메인쿼리가 
 * 				송은희의 EMP_NAME, JOB_CODE, SALARY를 반환
 * 				--> RESULT SET에 포함됨
 *
 * */
...
/*
 * 메인쿼리 조회했을때 선동일/J1/800만
 * 서브쿼리가 J1의 평균 구함
 * 메인쿼리 JOB_CODE, 서브쿼리 JOB_CODE가 같을때 (J1 = J1)
 * 선동일의 급여가(메인쿼리) 평균급여(서브쿼리) 보다 큰지 판별
 *
**/
...
SELECT EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 4850000
AND JOB_CODE = 'J2'; -- 송종기 조회

SELECT EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 3600000
AND JOB_CODE = 'J3'; -- 정중하 조회
...
FROM EMPLOYEE 
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
/*부서가 없어도 전체 직원 조회 결과에 
포함되어야하므로 LEFT JOIN함.

이 메인쿼리에서 조회한 직원들 중 누군가의 사수인 직원 찾기
우선 방명수부터 누군가의 사수인지 조회함..
**/
...
SELECT EMP_ID 
FROM EMPLOYEE
WHERE MANAGER_ID = 207; -- 방명수 EMP_ID(214)가 누군가의 MANAGER_ID 인지 조회
/* 사수가 214번인 사원의 EMP_ID가 
 * 216, 217 임
 * 이 번호들이 메인쿼리의 EMP_ID에 존재하면 조회결과에 포함
 * 
 * 다시 메인쿼리 조회해서 보면 216, 217 모두 존재하는 사원임.
 * 그러면 RESULT SET에 포함함
 * 
 * ----------------------------
 * 
 * 사수가 207(하이유)번인 사원의 EMP_ID가 
 * 206, 208, 209, 210 임
 * 이 번호들이 메인쿼리의 EMP_ID에 존재하면 조회결과에 포함
 * 
 * 다시 메인쿼리 조회해서 보면 206, 208, 209, 210 모두 존재하는 사원임.
 * 그러면 RESULT SET에 포함함
 *
 * */
 ...
 * 실제 데이터 값을 반환하지는 않습니다. 
 * 즉, 각 직원의 MANAGER_ID가 다른 직원의 EMP_ID와 일치하는 경우만 확인됨.
 * 
 * --> 서브쿼리 결과가 메인쿼리의 행들 중 일치하는게 있나?? TRUE 
 * 		(확인만 할 뿐 서브쿼리 결과 반환 X)
 ...
 * 해당 직원의 정보를 반환함.
 * 
 * --> 서브쿼리결과가 데이터를 반환함 (207, 208.. 같은)
 * --> 그게 메인쿼리의 MANAGER_ID에 있는지 확인 후 
 * 			메인쿼리에서 해당 직원의 정보를 반환함
 * */			 
			 

 /*
1. 메인쿼리는 직원 테이블 (EMPLOYEE)의 EMP_ID, EMP_NAME, DEPT_TITLE 및 MANAGER_ID 열을 선택.
2. LEFT JOIN 절은 EMPLOYEE와 DEPARTMENT 테이블을 DEPT_CODE와 DEPT_ID를 기준으로 조인. 
		이는 직원이 속한 부서의 정보를 가져오기 위함.
3. WHERE 절은 EXISTS 서브쿼리를 사용하여 특정 직원이 다른 직원의 관리자인지를 확인.
4. 서브쿼리는 EMPLOYEE 테이블을 다시 참조하고 있으며, 
		메인쿼리의 EMPLOYEE 테이블과 서브쿼리의 EMPLOYEE 테이블을 연결.
		그리고 서브쿼리에서는 EMP_ID가 현재 행의 MANAGER_ID와 같은 직원이 있는지를 확인.
5. 결과적으로 외부 쿼리는 조건을 만족하는 각 직원의 
		EMP_ID, EMP_NAME, DEPT_TITLE, MANAGER_ID를 반환.
* */
...
-- 메인쿼리(사번, 이름, 부서명(NULL이면 '소속없음'), 직급명, 입사일을 조회, 
-- 					단, 퇴사한 직원은 제외하고 조회)
...
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE ENT_YN = 'N';
/* 1. 방명수/D1/2010-04-04 00:00:00.000
 * 2. 차태연/D1/2013-03-01 00:00:00.000
 * 3. 전지연/D1/2007-03-20 00:00:00.000
 * */

-- 서브쿼리(부서별 입사일이 가장 빠른 사원 조회)
SELECT MIN(HIRE_DATE)
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1';
/* 1. 방명수가 있는 D1부서 가장빠른 입사일 조회해봤는데,
 * 		(2007-03-20 00:00:00.000)
 * 		방명수 입사일이랑 다름. 방명수 아님!(RESULT SET제외)
 * 2. 차태연 입사일이랑 다름. 차태연 아님!(RESULT SET제외)
 * 3. 전지연 입사일이랑 같음. 전지연임! (RESULT SET포함)
 * */
...
-- ** 서브쿼리에 GROUP BY 안써도 됨 (GROUP BY의 효과도 같이낸다)**

/* GROUP BY만으로는 못하나요?
 * 
 * GROUP BY를 사용하면 각 부서별로 최소 HIRE_DATE를 찾을 수는 있지만, 
 * 각 부서별로 최소 HIRE_DATE를 가진 사원의 
 * EMP_ID나 EMP_NAME을 얻는 것은 어렵다.
 * --> 서브쿼리 없이는 이를 단순히 GROUP BY 만으로 해결할 수 없음
 * 
 * */
...
/*
 *  스칼라라는게 단일이라는 뜻임. 하나 라는뜻 
		수학이나 프로그래밍에서 쓰는 학질적인 용어이다. 
		데이터베이스에서는 하나 라는 뜻으로 쓴다. 
		값 하나만 나오는 서브쿼리라고해서 단일행 서브쿼리같은것임. 
		근데 대신에 어디다가 쓰는거다? 
		--> SELECT 절에만 쓰는 단일행 서브쿼리를 '스칼라 서브쿼리'라고함.
